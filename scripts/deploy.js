...
function deploy () {
  ...
  if (env === 'development') {
    console.log(
      chalk.red(
        'Error: Please specify a deployment target other than development using NODE_ENV'
      )
    )
    return
  }
  ...
  if (!deployConfig.targets[env]) {
    console.log(
      chalk.red(
        'Error: Please specify a deployment target that exists in deploy_config.json using NODE_ENV'
      )
    )
    return
  }
  ...
  const copyCmd = `rsync -e 'ssh -p ${
    sshConfig.port
  }' --checksum --archive --compress --delete --keep-dirlinks --safe-links ${
    paths.build
  } ${(targetConfig.user && targetConfig.user + '@') || ''}${sshConfig.host}:${
    targetConfig.root
  }current/`
