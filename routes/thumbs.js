...
const createFilter = function({subjectFilter, agentFilter, dateFromFilter, dateToFilter, hasFeedbackFilter, notHandledFilter}) {
  const filter = {};
...
module.exports = function(debug) {
  ...
  router.get('/list', function(req, res, next) {
    ...
    const filter = createFilter({subjectFilter, agentFilter, dateFromFilter, dateToFilter, hasFeedbackFilter, notHandledFilter});

    const defList = Q.defer();
    Thumb.paginate(filter, { page, limit: PER_PAGE }, function(err, result) {
      if (err) { return next(err); }
      return defList.resolve({pages: result.pages, thumbs: result.docs, count: result.total});
    }
    , {sortBy: {createdAt: -1}});
    ...
    const lastWeekStart = moment().utc().subtract(1, 'week').startOf('week').toDate();
    const lastWeekEnd = moment().utc().subtract(1, 'week').endOf('week').toDate();
    ...
    return Q.all(promises).spread(({pages, thumbs, countAll}, countPos, countNeg, countPosWeek, countNegWeek) =>
      res.render('index', {
        thumbs,
        countAll,
        countPos,
        ...
        getSubjectLink: hooks.displaySubjectLink || (thumb => 'javascript:void(0)'),
        formatDate(date) {
  ...
  router.post('/feedback', function(req, res, next) {
    ...
    if (hooks.feedbackSaved) {
      return Q(Thumb.findOne({_id: req.body.id}).exec())
