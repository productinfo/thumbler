...
gulp.task("livereload", () => {
  ...
  livereload.listen();
  return gulp.watch(paths.app).on("change", file => setTimeout((() => livereload.changed(file.path)), 500));
});
...
gulp.task("deploy", ['build'], function() {
  ...
  if (!targetConfig.root.match("/$")) { targetConfig.root += "/"; }
  ...
  if (gutil.env.b) { bumpVersion(gutil.env.b); }

  const deployStart = Date.now();


  const sshCmd = `ssh ${(targetConfig.user && (targetConfig.user+'@')) || ''}${targetConfig.host} -p ${targetConfig.port}`;

  return gulp.src("")
    .pipe(shell([
      sshCmd + ` \"cd ${targetConfig.root}; mkdir -p current; rm -rf previous; cp -r current previous\"`,
      `rsync -e 'ssh -p ${targetConfig.port}' --checksum --archive --compress --delete --safe-links dist/ ${(targetConfig.user && (targetConfig.user+'@')) || ''}${targetConfig.host}:${targetConfig.root}current/`,
      sshCmd + " \"sudo toggl_thumbler_restart\""
    ])).pipe(tap(function() {
      const time = Date.now() - deployStart;
      return gutil.log(cl.green("Successfully deployed to ") + cl.yellow(env) + cl.green(" in ") + cl.yellow((time / 1000).toFixed(2) + " seconds"));
    })
  );
});
...
var bumpVersion = (type = 'patch') => {
  ...
  .on("end", () =>
    gulp.src("")
      ...
      ], {ignoreErrors: true})).pipe(tap(() => gutil.log(cl.green("Version bumped to ") + cl.yellow(version) + cl.green(", don't forget to push!")))
    )
  );
};
